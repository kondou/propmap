...
if not files:
    print(f"RBN JSONが見つかりません: {pattern}")
    sys.exit(1)

for fpath in files:
    d = json.load(open(fpath))
    if not d.get("records"):
        print(f"{Path(fpath).name}: レコードなし")
        continue
    steps = [r[3] for r in d["records"]]
    print(f"{Path(fpath).name}:")
    print(f"  t_step min={min(steps)} max={max(steps)} レコード数={len(steps)}")
    ...
    print(f"  t_step>=144のレコード数: {over144} ({over144/len(steps)*100:.1f}%)")
